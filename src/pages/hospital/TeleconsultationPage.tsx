...
const DEFAULT_MEET_URL = 'https://meet.google.com/tfx-psfd-xjd';
...
export const TeleconsultationPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    notes: DEFAULT_MEET_URL
  });

  const getMeetLink = (notes?: string) => {
    if (!notes) return DEFAULT_MEET_URL;
    const match = notes.match(/https?:\/\/[^\s]+/i);
    return match ? match[0] : DEFAULT_MEET_URL;
  };
      ...
      headerAction={
        <div className="flex items-center gap-2.5">
          <a
            ...
            className="inline-flex items-center gap-2 px-3.5 py-2.5 rounded-xl font-bold text-xs sm:text-sm text-white bg-emerald-600 hover:bg-emerald-700 shadow-md shadow-emerald-600/20 transition-all"
            title="Open Google Meet Chamber in New Tab"
          >
    ...
    >
      <div className="space-y-6">
        ...
        {activeCallSession ? (
          <div className="bg-slate-900 text-white rounded-3xl p-6 shadow-2xl border border-slate-700 space-y-6 animate-fade-in">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-800">
              ...
              {/* Network Status & Google Meet Direct Join */}
              <div className="flex flex-wrap items-center gap-2.5">
                <a
                  href={getMeetLink(activeCallSession.notes)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1.5 text-xs text-white bg-emerald-600 hover:bg-emerald-500 px-3.5 py-1.5 rounded-xl font-bold border border-emerald-500 shadow-md shadow-emerald-600/30 transition-all"
                >
                ...
                >
                  {isLowBandwidthMode ? 'Low Bandwidth: ON' : 'Low Bandwidth: Auto'}
                </button>
              </div>
            </div>
                ...
                <div className="relative aspect-video rounded-2xl bg-slate-950 border border-slate-800 overflow-hidden flex items-center justify-center shadow-inner">
                  ...
                  <div className="flex flex-col items-center justify-center text-center p-6 space-y-3">
                    ...
                    <p className="text-xs text-slate-400 max-w-sm">
                      Assisted by Community Health Officer (CHO) at PHC Khed Sub-Centre.
                    </p>
                    <div className="flex items-center gap-2 pt-1">
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-500 shadow-lg shadow-emerald-600/30 transition-all"
                      >
                        <Video className="w-4 h-4" />
                        <span>Launch Live Google Meet Video</span>
                        <ExternalLink className="w-3.5 h-3.5" />
                      </a>
                    </div>
        ...
        <div className="bg-white dark:bg-brand-dark-surface rounded-2xl border border-slate-200/80 dark:border-brand-dark-border p-5 sm:p-6 shadow-xs space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100 dark:border-brand-dark-border">
            <div>
            ...
            {/* Quick Google Meet Access Pill */}
            <div className="flex items-center gap-2 text-xs bg-purple-50 dark:bg-purple-950/50 border border-purple-200 dark:border-purple-800/60 px-3 py-1.5 rounded-xl">
              <span className="font-semibold text-purple-800 dark:text-purple-300">Live Meet Link:</span>
              <a
                href={DEFAULT_MEET_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="font-mono text-indigo-600 dark:text-indigo-400 underline hover:text-indigo-800 inline-flex items-center gap-1 font-bold"
              >
                <span>{DEFAULT_MEET_URL}</span>
                <ExternalLink className="w-3 h-3" />
              </a>
              <button
                type="button"
                onClick={() => copyMeetLink(DEFAULT_MEET_URL)}
                className="p-1 text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 ml-1"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {teleconsultations.map((session) => {
              const meetLink = getMeetLink(session.notes);
              const isUrl = session.notes && session.notes.startsWith('http');
                    ...
                    <div className="mt-2 text-xs">
                      {isUrl ? (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-1.5 text-indigo-600 dark:text-indigo-400 font-semibold underline hover:text-indigo-800 break-all"
                        >
                        ...
                        <p className="text-slate-600 dark:text-brand-dark-text">
                          {session.notes || 'Sub-Centre teleconsultation'}
                        </p>
                      )}
                    </div>
                  </div>

                  <div className="pt-3 border-t border-slate-200/60 dark:border-brand-dark-border/60 space-y-2">
                    <a
                      href={meetLink}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full py-2 px-3 rounded-xl text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 shadow-sm shadow-emerald-600/20 flex items-center justify-center gap-2 transition-all"
                    >
                      <Video className="w-3.5 h-3.5" />
                    ...
                    <button
                      ...
                      className="w-full py-2 px-3 rounded-xl text-xs font-semibold text-purple-700 dark:text-purple-300 bg-purple-50 dark:bg-purple-950/60 hover:bg-purple-100 dark:hover:bg-purple-900/60 border border-purple-200 dark:border-purple-800/60 flex items-center justify-center gap-2 transition-all"
                    >
                      <Video className="w-3.5 h-3.5" />
                      <span>Launch In-App Chamber</span>
                    </button>
                  </div>
                </div>
              );
            ...
            <form onSubmit={handleScheduleSubmit} className="space-y-3.5">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="block text-xs font-bold text-slate-700 dark:text-brand-dark-text">
                    Sub-Centre / Connection Notes & Google Meet Link
                  </label>
                  <a
                    href={formData.notes || DEFAULT_MEET_URL}
                    target="_blank"
                    rel="noopener noreferrer"
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-xs rounded-xl bg-slate-50 dark:bg-brand-dark-elevated border border-slate-200 dark:border-brand-dark-border font-mono text-purple-700 dark:text-purple-300"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100 dark:border-brand-dark-border">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 text-xs font-bold text-white bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 rounded-xl shadow-xs"
                >
                  Confirm Video Slot
                </button>
              </div>
